...
export class EmailFallbackStrategy {
    ...
    async sendWithFallback(
        ...
        if (emailConnectors.length === 0) {
            this.logger.error(`No email providers configured for organization: ${organizationId}`);
            return { messageId: null, status: 'failed', error: 'No email providers configured' };
        }

        // 2. Try each provider until one succeeds
        for (const connector of emailConnectors) {
            try {
                this.logger.log(`Attempting to send email via ${connector.type} (Connector: ${connector.id})`);

                // Get decrypted config
                const fullConnector = await this.connectorsService.findOne(connector.id, organizationId);
                const provider = EmailProviderFactory.create(fullConnector.type, fullConnector.config);
                ...
                this.logger.error(`Error using provider ${connector.type}: ${error.message}`);
            }
        }

        return { messageId: null, status: 'failed', error: 'All email providers failed' };
    }
}
