import { Controller, Post, Body, Logger, Get, NotFoundException } from '@nestjs/common';
...
export class TestBenchController {
    ...
    async triggerSeoAlert(@Body() body: { projectId: string; type: string }) {
        this.logger.log(`🧪 Triggering SEO Alert test for project ${body.projectId}`);
        ...
        try {
            ...
            if (body.type === 'rank_drop') {
                ...
                await this.alerts.createAlert(body.projectId, orgId, {
                    ...
                    message: '🔔 Competitor "CompetitorInc" published a new blog post: "10 SEO Tips"',
                    metadata: {
    ...
    async triggerCompetitorCheck(@Body() body: { projectId: string; url: string }) {
        ...
        try {
            ...
            if (!competitor) {
                competitor = await this.competitors.createCompetitor(body.projectId, orgId, {
                    domain: body.url
                });
            }

            // Trigger check
            const result = await this.competitors.checkCompetitorRankings(competitor.id, orgId);
        ...
        try {
            ...
            for (const data of companiesData) {
                const company = await (this.prisma as any).company.upsert({
                    ...
                    create: { ...data, id: `mock - company - ${data.domain} `, organizationId: org.id },
                });
    ...
    async triggerGscSync(@Body() body: { projectId: string }) {
        ...
        const job = await this.gsc.triggerSync(body.projectId, project.organizationId);
