...
export class AiService {
    ...
    async callOpenAI(prompt: string, systemPrompt?: string, jsonMode = true) {
        if (!this.openAiKey || this.openAiKey.includes('dummy')) {
            this.logger.warn('Using High-Fidelity AI Simulation (Dummy Key or No Key)');
            ...
            if (jsonMode) {
                if (prompt.includes('Enrich')) {
                    return {
                        ...
                        description: "A cutting-edge tech company focused on building the next generation of business operating systems."
                    };
                }
                return {
                    name: "AI Generated Template",
                    content: "Subject: Elevating your workflow with ORI-OS \n\nHello, \n\nI noticed your interest in scaling your production environment. [Simulated AI Response]"
                }
            }
