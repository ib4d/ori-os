...
export class SeoKeywordsService {
    ...
    async getRankingHistory(keywordId: string, organizationId: string, limit = 30) {
    ...
    async clusterKeywords(projectId: string, organizationId: string, keywords: string[]) {
        this.logger.log(`🤖 Clustering ${keywords.length} keywords for project: ${projectId}`);
        ...
        const clusters = await this.ai.callOpenAI(prompt, 'You are an SEO intent analysis expert.', true);
