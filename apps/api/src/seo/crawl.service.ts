...
export class CrawlService {
    ...
    async startCrawl(projectId: string, organizationId: string, maxPages?: number) {
    ...
    async getIssues(crawlId: string, organizationId: string, filters?: any) {
        ...
        return {
            ...
            total,
            summary: {
                critical: summary.find((s: any) => s.severity === 'critical')?._count || 0,
                warning: summary.find((s: any) => s.severity === 'warning')?._count || 0,
    ...
    async updateIssueStatus(issueId: string, organizationId: string, status: string) {
