...
export class ContentAnalysisService {
    ...
    private async analyzePage(url: string, keyword: string) {
        try {
            ...
            const keywordInMeta = metaDescription.toLowerCase().includes(keyword.toLowerCase());
            ...
            console.error(`[Content Analysis] Failed to analyze ${url}:`, error.message);
            return {
                url,
                error: error.message,
                wordCount: 0,
    ...
    private generateRecommendations(yourPage: any, competitors: any[]) {
        ...
        if (yourPage.keywordDensity < 0.5) {
            ...
            recommendations.push({
                ...
                recommendation: 'Reduce keyword usage to avoid over-optimization penalty',
            });
        }
    ...
    async getAnalyses(projectId: string, organizationId: string, filters?: any) {
        ...
        const total = await (this.prisma as any).sEOContentAnalysis.count({ where });
