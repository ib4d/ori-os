...
export class RankingsService {
    ...
    async checkRankings(projectId: string, organizationId: string, keywordIds?: string[]) {
    ...
    async getRankingSummary(projectId: string, organizationId: string, days: number = 30) {
        // Verify project belongs to organization
        const project = await (this.prisma as any).sEOProject.findFirst({
            where: {
                id: projectId,
        ...
        const improved = rankings.filter(r => r.prevPosition && r.position < r.prevPosition).length;
        const declined = rankings.filter(r => r.prevPosition && r.position > r.prevPosition).length;
        const unchanged = rankings.filter(r => r.prevPosition && r.position === r.prevPosition).length;
