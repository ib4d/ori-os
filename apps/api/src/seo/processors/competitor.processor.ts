...
export class SEOCompetitorProcessor extends WorkerHost {
    ...
    async process(job: Job<any>): Promise<any> {
        ...
        try {
            ...
            if (!response.ok) throw new Error(`Failed to fetch ${competitor.domain}: ${response.statusText}`);
            ...
            if (competitor.metadata && (competitor.metadata as any).lastTitle && (competitor.metadata as any).lastTitle !== title) {
                changes.push(`Title changed: "${(competitor.metadata as any).lastTitle}" -> "${title}"`);
            }
            ...
            console.error(`[SEO Competitor] Error processing ${competitorId}:`, error);
            throw error;
        }
    }
}
