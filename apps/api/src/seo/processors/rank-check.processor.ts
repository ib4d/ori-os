...
export class SEORankCheckProcessor extends WorkerHost {
    ...
    async process(job: Job<any>): Promise<any> {
        ...
        console.log(`[SEO Rank Check] Checking ranks for project ${projectId}, ${keywordIds?.length || 'all'} keywords`);
        ...
        try {
            ...
            console.log(`[SEO Rank Check] Found ${keywords.length} keywords to check`);
            ...
            for (const keyword of keywords) {
                try {
                    ...
                    results.push({
                        ...
                        position: rankData.position,
                        change: keyword.lastPosition ? keyword.lastPosition - rankData.position : null,
                    });

                    // Add delay to avoid rate limiting in production
                    await new Promise(resolve => setTimeout(resolve, 100));
                } catch (error) {
                    console.error(`[SEO Rank Check] Error checking keyword ${keyword.id}:`, error);
                }
            }

            console.log(`[SEO Rank Check] Completed rank check for project ${projectId}`);
    ...
    private async createRankAlert(projectId: string, keyword: any, change: number, type: 'drop' | 'top3' | 'top10'): Promise<void> {
        try {
