...
export class SeoKeywordsController {
    ...
    async getRankingHistory(
        ...
        return this.seoKeywordsService.getRankingHistory(id, organizationId, limitNum);
    }
    ...
    @Post('discover')
    async discover(@Body() body: { query: string; lang?: string; country?: string }) {
        return this.seoKeywordsService.discoverKeywords(body.query, body.lang, body.country);
    }

    @Post('cluster')
    async cluster(@Body() body: { projectId: string; keywords: string[] }, @Req() req: any) {
        const organizationId = req.user?.organizationId || 'default-org';
        return this.seoKeywordsService.clusterKeywords(body.projectId, organizationId, body.keywords);
    }
}
