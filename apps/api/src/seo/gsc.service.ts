...
export class GSCService {
    ...
    private readonly REDIRECT_URI = process.env.GOOGLE_REDIRECT_URI || 'http://localhost:3000/api/seo/gsc/callback';
    ...
    async handleCallback(code: string, projectId: string) {
        try {
            ...
            this.logger.error('GSC OAuth callback error:', error.response?.data || error.message);
    ...
    async syncProjectData(projectId: string) {
        try {
            ...
            if (new Date() >= new Date(project.gscTokenExpiresAt)) {
                accessToken = await this.refreshAccessToken(projectId, project.gscRefreshToken);
            }
            ...
            this.logger.error(`GSC sync error for project ${projectId}:`, error.message);
            throw error;
        }
    }

    private async refreshAccessToken(projectId: string, refreshToken: string): Promise<string> {
        try {
            ...
            this.logger.error(`Failed to refresh GSC token for project ${projectId}:`, error.response?.data || error.message);
    ...
    private async fetchGSCData(
        ...
        if (process.env.GSC_SIMULATION === 'true' || accessToken === 'mock-token') {
            ...
            return [
                { keys: ['seo studio', 'https://example.com/', 'desktop', startDate.toISOString()], clicks: 120, impressions: 1500, ctr: 0.08, position: 2.4 },
                { keys: ['ori os', 'https://example.com/pricing', 'mobile', startDate.toISOString()], clicks: 45, impressions: 800, ctr: 0.05, position: 5.1 },
                { keys: ['nextjs seo', 'https://example.com/blog', 'desktop', startDate.toISOString()], clicks: 12, impressions: 2000, ctr: 0.006, position: 12.3 },
                { keys: ['automated seo', 'https://example.com/', 'tablet', startDate.toISOString()], clicks: 5, impressions: 100, ctr: 0.05, position: 1.2 },
            ];
        }
